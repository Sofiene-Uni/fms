...
def plot_combined(jssp, show_rank=False, format_="jpg", dpi=300):
    renders_folder = f"{os.getcwd()}\\renders\\"
    if not os.path.exists(renders_folder):
        os.makedirs(renders_folder)
    ...
    for token in finished_tokens:
        for place, entry in token.logging.items():
            if place in jssp.filter_nodes("agv"):
...
def plot_agv(jssp,format_="jpg" ,dpi=300): 
    ...
    for token in finished_tokens:
        for place, entry in token.logging.items():
            if place in jssp.filter_nodes("agv"):
                ...
                data_dict["agv_id"].append(f"AGV- {jssp.places[place].color}")
                data_dict["entry_values"].append(entry[0])
                data_dict["process_times"].append(entry[2])
                data_dict["jobs"].append(token.color[0])
    ...
    for bar, rank in zip(bars, data_dict["jobs"]):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_y() + bar.get_height() / 2, 
                f'{rank}', ha='center', va='center', color='black', fontsize=16)
    ...
    ax.tick_params(axis='x', labelsize=16) 
    ax.tick_params(axis='y', labelsize=16) 
    ax.set_title(f"Agvs Schedule for instance {jssp.instance_id}  : {jssp.n_jobs} jobs , {jssp.n_machines} machines, {jssp.n_agv} AGV ", fontsize=18, fontweight='bold')
...
def plot_debug(jssp, show_rank=False, format_="jpg", dpi=300):
    renders_folder = f"{os.getcwd()}\\renders\\"
    if not os.path.exists(renders_folder):
        os.makedirs(renders_folder)
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(18, 12), sharex=True)  # Increased figure size
    ...
    ax2.set_xticks(range(0, int(jssp.clock) + 1))
